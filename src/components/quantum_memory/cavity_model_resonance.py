...
class LambdaCavityModel(QuantumMemory):
    ...
    def solve_S_full_second_order(self, T, C, Delta, gamma=1.0, time_steps=1000, verbose=False):
        """
        Solve the full second-order ODE for S(t), including the double derivative,
        based on the exact coupled equations (no adiabatic elimination).
        """
        ...
        E_in_vals = E_in(t_vals)

        # Compute Omega(t)
        gamma_eff = gamma * (1 + C)
        denom = np.sqrt(2 * gamma * (1 + C))
        integral = cumulative_trapezoid(np.abs(E_in_vals)**2, t_vals, initial=0)
        Omega_vals = - ((gamma_eff - 1j * Delta) / denom) * (E_in_vals / np.sqrt(integral + 1e-12))
        Omega_sq_vals = np.abs(Omega_vals)**2
        dOmega_vals = np.gradient(Omega_vals, t_vals)
        ...
        def rhs(t, Y):
            ...
            dOmega = dOmega_vals[idx]
            E = E_in_vals[idx]

            # From the second-order ODE
            frac_term = (dOmega.conj() / (Omega.conj() + 1e-12)) * dS
            ddS = frac_term - (gamma_eff + 1j*Delta) * dS - np.abs(Omega_sq_vals[idx])**2 *S - np.conj(Omega)*np.sqrt(2*gamma*C)*E 

            return [dS.real, dS.imag, ddS.real, ddS.imag]

        # Initial condition: S(0)=0, dS(0)=0
        y0 = [0, 0, 0, 0]

        # Solve system
        sol = solve_ivp(rhs, [0, T], y0, t_eval=[T])
        S_final = sol.y[0][0] + 1j * sol.y[1][0]

        # Compute efficiency
        eta_storage = np.abs(S_final)**2
        eta_total = eta_storage * (C / (1 + C))

        if verbose:
            print(f"S(T) = {S_final}")
            print(f"Storage efficiency = {eta_storage}")
            print(f"Total efficiency (retrieval included) = {eta_total}")
    ...
    def plot_efficiency_sweep(self, title: str, save_filename: str = 'cavity_model_efficiency_sweep_figure.png', cooperativity_vals: list = []):
        ...
        for C in cooperativity_vals:
            ...
            for TCgamma in TCgamma_vals:
                ...
                # eta = self.solve_S_full_second_order(T, C, Delta=self._Delta)
    ...
    def plot_efficiency_sweep_Delta(self, title: str, save_filename: str = 'cavity_model_efficiency_sweep_figure.png', Delta_vals: list = []):
        ...
        for Delta in Delta_vals:
            ...
            for TCgamma in TCgamma_vals:
                T = TCgamma / (C * self._gamma)
                eta = self._total_efficiency(T, C, Delta=Delta, storage_decay=self._storage_decay)
                eta_totals.append(eta)
